...
# ═══════════════════════════════════════════════════════════════════════════
app = Flask(__name__)
app.config['SECRET_KEY']                 = os.environ.get('SECRET_KEY', 'exampro-sug-2026-xyz')
...
def answer_checking(exam_id):
    return render_template('answer_checking.html', exam_id=exam_id)
    ...
    try:
        ...
        if not ROLL_REGEX.match(roll_number):
            return jsonify({'error':
                'Roll number must be exactly 11 digits (e.g. 23014168059)'}), 400
        if not password or len(password) < 6:
            return jsonify({'error':
                'Password must be at least 6 characters'}), 400
        if users_col.find_one({'roll_number': roll_number, 'role': 'student'}):
            return jsonify({'error':
                f'Roll number {roll_number} is already registered'}), 409
...
def register_faculty():
    try:
        ...
        if secret_key != FACULTY_REG_KEY:
            print(f"[SECURITY] ❌ Wrong faculty key from {request.remote_addr}")
            return jsonify({'error':
        ...
        if not password or len(password) < 8:
            return jsonify({'error':
                'Faculty password must be at least 8 characters'}), 400
        if users_col.find_one({'username': username, 'role': 'faculty'}):
            return jsonify({'error':
...
def api_login():
    try:
        ...
        if not user.get('is_active', True):
            return jsonify({'error':
                'Account is deactivated. Contact Admin.'}), 403

        stored = user.get('password', '')
        if not stored:
            return jsonify({'error':
                'No password set for this account'}), 500
        ...
        session['username']    = str(
            user.get('username') or user.get('roll_number', ''))
...
def get_exams():
    ...
    exams = list(exams_col.find(query).sort('start_time', ASCENDING))
...
def create_exam():
    try:
        ...
        try:
            scheduler.add_job(auto_start_exam, 'date',
                run_date=doc['start_time'], args=[exam_id],
                id=f'start_{exam_id}', replace_existing=True)
            scheduler.add_job(auto_end_exam, 'date',
                run_date=doc['end_time'], args=[exam_id],
                id=f'end_{exam_id}', replace_existing=True)
        except Exception as e:
...
def stop_exam(exam_id):
    ...
    for ans in active:
        answers_col.update_one({'_id': ans['_id']}, {'$set': {
            'submitted':      True,
            'submit_time':    utcnow(),
            'auto_submitted': True
        }})
...
def delete_exam(exam_id):
    ...
    try: scheduler.remove_job(f'start_{exam_id}')
    except Exception: pass
    try: scheduler.remove_job(f'end_{exam_id}')
    except Exception: pass
...
# ═══════════════════════════════════════════════════════════════════════════
# AI CORE — Question Parse + Model Answer Generate
# ══════════════════════════════════��════════════════════════════════════════
def get_groq_client():
    if GROQ_OK and GROQ_KEY:
...
def ai_parse_questions(text, subject='General'):
    """
    Groq AI se questions extract karo + model answers generate karo.
...
[
  {{
    ...
    "model_answer": "The correct answer is Option B because... detailed explanation..."
  }},
  {{
    ...
    "options": [],
    "correct_option": "",
    "model_answer": "Complete detailed answer covering all key points: 1. Point one explained... 2. Point two explained... 3. Examples and diagrams description..."
  }}
]

Strict Rules:
- type MUST be "mcq" if options (a/b/c/d or 1/2/3/4) exist in text
- type MUST be "subjective" for descriptive/explain/define questions
- options = extract option texts WITHOUT the a. b. c. d. prefix
- correct_option = exact text of correct answer (for mcq)
- model_answer = ALWAYS write detailed answer for ALL questions
- marks: 1 for MCQ, 2-5 for short answer, 5-10 for long answer
- Do NOT skip any question
- If correct option not mentioned, set correct_option = ""
...
JSON array only:"""
    ...
    try:
        ...
        if start == -1 or end <= start:
            print("[AI Parse] No JSON array in response")
            return None
        ...
        for q in questions:
            ...
            if not txt or len(txt) < 5:
                continue
            qtype = str(q.get('type', 'subjective')).lower()
        ...
        print(f"[AI Parse] ✅ {len(cleaned)} questions extracted")
...
def ai_generate_model_answers(questions, subject='General'):
    """
    Already parsed questions ke liye model answers generate karo.
    Use karo jab ai_parse_questions mein model_answer empty ho.
    """
    client_ai = get_groq_client()
    if not client_ai:
        return questions

    # Find questions with empty model answers
    need_answers = [
    ...
    print(f"[AI Answers] Generating answers for "
    ...
    for batch_start in range(0, len(need_answers), batch_size):
        ...
        q_text = '\n\n'.join([
            f"Q{idx+1} ({q['type']}, {q['marks']} marks): {q['text']}"
            + (f"\nOptions: {', '.join(q['options'])}"
               if q.get('options') else '')
            for idx, (_, q) in enumerate(batch)
        ])

        prompt = f"""You are an expert {subject} professor.
Write detailed model answers for these exam questions.

{q_text}

Return ONLY a JSON array with answers in same order:
[
  {{
    "answer": "Complete detailed model answer for Q1..."
  }},
  {{
    "answer": "Complete detailed model answer for Q2..."
  }}
]

Rules:
- Answer must be comprehensive and exam-ready
- For MCQ: explain why correct option is right
- For subjective: cover all key points with examples
- No extra text, only JSON array"""
        ...
        try:
            ...
            for j, (orig_idx, _) in enumerate(batch):
                if j < len(answers):
                    ans = str(answers[j].get('answer', '')).strip()
                    if ans:
                        questions[orig_idx]['model_answer'] = ans
...
def fallback_parse_questions(text):
    """AI fail hone par basic text parsing"""
    lines   = text.split('\n')
    result  = []
    cur     = None
    opt_buf = []
...
def upload_file(exam_id):
    """
    ★ MAIN UPLOAD ROUTE ★
    1. File se text extract karo
    2. AI se questions + model answers generate karo
    3. JSON return karo → frontend directly use kare
    """
        ...
        if fname.endswith('.pdf'):
            ...
            if not DOCX_OK:
                return jsonify({'error':
                    ...
                    'Run: pip install python-docx'}), 500
    ...
    print(f"[UPLOAD] '{f.filename}' → "
          f"{len(text)} chars for exam {exam_id}")

    # ── Step 2: Get exam subject ──────────────────────────────
    exam    = exams_col.find_one({'exam_id': exam_id})
    subject = (exam.get('subject') or 'General') if exam else 'General'

    # ── Step 3: AI Parse ──────────────────────────────────────
    questions = ai_parse_questions(text, subject)

    if questions:
        # Check if any model answers are empty
        empty_answers = sum(
            1 for q in questions
            if not q.get('model_answer', '').strip()
        )
        if empty_answers > 0:
            print(f"[AI] {empty_answers} empty answers — "
                  f"generating...")
            questions = ai_generate_model_answers(
                questions, subject)

        print(f"[UPLOAD] ✅ AI: {len(questions)} questions "
              f"with model answers")
        return jsonify({
            'questions': questions,
            'count':     len(questions),
            'ai':        True,
            'subject':   subject,
            'message':   (f'AI extracted {len(questions)} '
                          f'questions with model answers')
        })

    # ── Step 4: Fallback ──────────────────────────────────────
    print("[UPLOAD] AI failed — using fallback parser")
...
def get_questions(exam_id):
    ...
    if session['role'] == 'student':
        ...
        for q in qs:
            ...
            if exam.get('randomize_o') and q.get('options'):
                opts = q['options'][:]
                rng.shuffle(opts)
                q['options'] = opts
            result.append(q)
...
# ═══════════════════════════════════════════════════════════════════════════
@app.route('/api/exams/<exam_id>/session', methods=['POST'])
@role_required('student')
...
@app.route('/api/exams/<exam_id>/autosave', methods=['POST'])
@role_required('student')
...
def auto_grade(exam_id, user_id):
    ...
    qs      = list(questions_col.find({'exam_id': exam_id}))
...
def get_submissions(exam_id):
    ...
    for s in subs:
        ...
        u   = users_col.find_one({'_id': oid}) if oid else None
        s['student_name'] = u.get('name', 'N/A') if u else 'N/A'
        s['roll_number']  = u.get('roll_number', '') if u else ''
        out.append(s)
    return jsonify(out)
...
def update_marks(exam_id, user_id):
    ...
    marks_col.update_one(
        {'exam_id': exam_id, 'user_id': user_id},
        {'$set': {
            'manual_marks': int(data.get('manual_marks', 0)),
...
def publish_results(exam_id):
    ...
    for i, r in enumerate(rows):
        ...
        results_col.update_one(
            {'exam_id': exam_id, 'user_id': r['user_id']},
            {'$set': {
                'exam_id':      exam_id,
                'user_id':      r['user_id'],
                'total':        r['total'],
                ...
                'pass':         r['total'] >= (
                    exam.get('pass_marks', 0) if exam else 0),
                'published':    True,
                'published_at': utcnow()
            }},
            upsert=True
    ...
    socketio.emit('result_published', {'exam_id': exam_id})
    return jsonify(
        {'message': f'Results published for {n} students'})
...
def get_results():
    if session['role'] == 'student':
        ...
        docs = list(results_col.find({'published': True}))
...
def get_exam_results(exam_id):
    ...
    for d in docs:
        ...
        u   = users_col.find_one({'_id': oid}) if oid else None
        d['name']        = u.get('name', '') if u else ''
        d['roll_number'] = u.get('roll_number', '') if u else ''
        out.append(d)
    return jsonify(out)
...
def analytics(exam_id):
    exam   = exams_col.find_one({'exam_id': exam_id})
    rdocs  = list(results_col.find({'exam_id': exam_id}))
    ...
    pm        = exam.get('total_marks', 100) if exam else 100
    buckets   = [0] * 10
    for s in scores:
...
# AI GRADING
# ═══════════════════════════════════════════════════════════════════════════
def ai_grade(question, answer, marks, subject='General'):
    ...
    if client_ai:
        try:
            prompt = f"""You are a strict but fair examiner for {subject}.

QUESTION ({marks} marks): {question}
STUDENT ANSWER: {answer}

Grade using your expert knowledge.
...
{{
  ...
  "key_points": ["<point>", "<point>"]
    ...
    # Keyword fallback
    stops = {'this','that','with','from','have','will',
    ...
    return {
        ...
        'confidence': 'low',
        'key_points': [
            f'{len(qw & aw)}/{len(qw)} key terms matched'],
...
def ai_grade_all(exam_id, user_id):
    try:
        ...
        return jsonify({
            'message':          (f'AI graded {len(results)}'
                                 f' questions'),
...
def get_ai_results(exam_id, user_id):
    ...
    return jsonify({
        ...
        'ai_graded_at':        (graded_at.isoformat()
                                if hasattr(graded_at,
                                           'isoformat')
                                else str(graded_at)),
        'ai_model':            (
            m.get('ai_results') or [{}])[0].get(
            'model', 'unknown'),
...
def admin_list_users():
    ...
    users = list(users_col.find(query, {'password': 0}))
    for u in users:
        u['_id'] = str(u['_id'])
        for field in ['created_at', 'last_login']:
            if isinstance(u.get(field), datetime.datetime):
